...
const solve = (set: TreeMultiSet<number>, k: number): number[] | false => {
  if (set.size() === 0) return []
  const max = set.rvalues().next().value as number
  const m2 = max - 2 * k
  if (set.has(m2)) {
    // console.log([...set.values()], min, max, m1, m2, k)
    set.delete(max)
    set.delete(m2)
    const tmp = solve(set, k)
    set.add(max)
    set.add(m2)
    if (tmp) {
      return [...tmp, max - k]
...
function recoverArray (nums: number[]): number[] {
  const set = new TreeMultiSet(nums)
  const set1 = new TreeMultiSet(nums)
  const max = set.rvalues().next().value as number
  for (const val of set.values()) {
    if ((max - val) % 2 === 0) {
      const k = (max - val) / 2
      // console.log(max, val, k)
      const tmp = solve(set1, k)
      if (tmp) {
        return tmp
      }
    }
